...
def map_view(request):
    if request.method == "POST":
        try:
            ...
            print(
                f"Position de l'utilisateur : lat={user_lat}, lon={user_lon}")
        ...
        for s in stations:
            ...
            try:
                ...
                if lat < -90 or lat > 90 or lng < -180 or lng > 180:
                    print(
                        f"Station ignorée (coordonnées hors limites) : {
                            s.nom}, latitude={lat}, longitude={lng}")
                    continue
            except (ValueError, TypeError):
                print(
                    f"Station ignorée (coordonnées invalides) : {
                        s.nom}, latitude={
                        s.latitude}, longitude={
                        s.longitude}")
                continue
            available = bool(
                s.disponibilite) if s.disponibilite is not None else True
            all_stations.append({
            ...
            print(
                f"Station ajoutée : {
                    s.nom}, lat={lat}, lng={lng}, available={available}, has_username={
                    s.username is not None}")
