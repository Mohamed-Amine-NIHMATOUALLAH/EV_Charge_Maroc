...
def create_checkout_session(request, station_id):
    ...
    if request.method == "POST":
        try:
            ...
            print(
                f"Données extraites - time_start: {time_start}, duration: {duration}, power: {power}")

            # Si les données ne sont pas dans request.POST, essayez de les
            # extraire du corps de la requête
            if not time_start or not duration or not power:
                try:
                    if 'application/json' in request.content_type:
                        ...
                        print(
                            f"Données extraites du JSON - time_start: {time_start}, duration: {duration}, power: {power}")
                except Exception as e:
                    print(
                        f"Erreur lors de l'extraction des données JSON: {
                            str(e)}")

            if not time_start or not duration or not power:
                return JsonResponse(
                    {'error': 'Veuillez spécifier la date de début, la durée et la puissance'}, status=400)

            # Convertir en objets datetime et float
            try:
                time_start = timezone.datetime.fromisoformat(
                ...
                return JsonResponse(
                    {'error': f'Erreur de format des données: {str(e)}'}, status=400)
            ...
            if conflicting_reservations.exists():
                return JsonResponse(
                    {'error': 'Ce créneau est déjà réservé'}, status=409)
            ...
            try:
                ...
                reservation.delete()
                return JsonResponse(
                    {'error': f'Erreur Stripe: {str(e)}'}, status=500)
