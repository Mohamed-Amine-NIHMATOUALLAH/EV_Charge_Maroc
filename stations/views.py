...
class station_List_Fournisseur(ListView):
    ...
    def get(self, request, username, *args, **kwargs):
        ...
        stations = station.objects.filter(
            username__username=username)  # Utilisez 'username__username'

        # Récupérer les photos et avis associés
        photos = photo.objects.filter(
            ID_Station__in=stations.values_list(
                'ID_Station', flat=True))
        avis = avi.objects.filter(
            ID_Station__in=stations.values_list(
                'ID_Station', flat=True))
...
def add_station(request, username):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = {
                'error': 'Latitude, longitude, puissance, and prix_kw must be valid numbers.'}
            return render(
                request, 'ADD_Station.html', {
                    'error': error, 'username': username})
        ...
        return redirect(
            reverse(
                'stations:station_List_Fournisseur',
                kwargs={
                    'username': username}))
...
def update_station(request, ID_Station, username):
    station_update = station.objects.get(
        ID_Station=ID_Station,
    ...
    if request.method == 'POST':
        ...
        if not all([adresse,
            ...
            error = {'error': 'All fields are required.'}
            return render(
                request, 'update_station.html', {
                    'error': error, 'station': station_update, 'username': username})
        ...
        if station.objects.filter(
                adresse=adresse).exclude(
                ID_Station=ID_Station).exists():
            error = {'error': 'A station with this address already exists.'}
            return render(
                request, 'update_station.html', {
                    'error': error, 'station': station_update, 'username': username})
        ...
        existing_station = station.objects.filter(
            latitude=latitude, longitude=longitude).exclude(
            ID_Station=ID_Station).first()
        if existing_station:
            if existing_station.username is None:
                # Supprimer la station existante avec username=None
                existing_station.delete()
                ...
                # Si username != None, afficher une erreur
                error = {
                    'error': 'A station with these coordinates already exists.'}
                return render(
                    request, 'update_station.html', {
                        'error': error, 'station': station_update, 'username': username})
    ...
    return render(
        request, 'update_station.html', {
            'station': station_update, 'username': username})
...
def station_List_ProprietaireVE(request):
    ...
    photos = photo.objects.filter(
        ID_Station__in=stations_list.values_list(
            'ID_Station', flat=True))

    # Récupérer tous les avis
    avis_list = avi.objects.filter(
        ID_Station__in=stations_list.values_list(
            'ID_Station', flat=True))
    ...
    if user.is_authenticated:
        ...
        if Fournisseur.objects.filter(username=user.username).exists():
            ...
            # Au lieu de:
            # print(f"Utilisateur identifié comme Fournisseur:
            # {user.username}")  # Débogage

            # Utiliser:
            logger.debug(
                f"Utilisateur identifié comme Fournisseur: {
                    user.username}")  # Débogage
        # Sinon, vérifier si c'est un ProprietaireVE
            ...
            print(
                f"Utilisateur identifié comme ProprietaireVE: {
                    user.username}")  # Débogage
...
def reservations(request, ID_Station, username):
    ...
    user_review = None
    if has_reservation:
        user_review = avi.objects.filter(
            ID_Station=station_obj, username=user).first()

    # Calcul de la note moyenne
    total_notes = reviews.count()
    note_moyenne = round(sum(r.note for r in reviews) /
                         total_notes, 1) if total_notes > 0 else 0
    ...
    try:
        if date_param:
            ...
            print(
                f"Date sélectionnée: {current_date} (à partir du paramètre: {date_param})")
    ...
    print(
        f"Date actuelle: {current_date}, Précédente: {prev_date}, Suivante: {next_date}")
...
def make_reservation(request, ID_Station, username):
    if request.method == 'POST':
        ...
        reservation.save()
        return redirect(
            'stations:reservation_confirmation',
...
def reservation_confirmation(request, reservation_id):
    ...
    station_obj = station.objects.get(
        ID_Station=reservation.ID_Station.ID_Station)
    # Process the reservation confirmation or any logic you need
    return render(request, 'reservation_confirmation.html', {
                  'reservation': reservation, 'station': station_obj})
...
def user_reservations(request, username):
    ...
    # Récupérer toutes les réservations de l'utilisateur, triées par date (les
    # plus récentes d'abord)
    reservations = Reservation.objects.filter(
        username=user).order_by('-time_start')
...
def edit_review(request, ID_Station, username):
    if request.method == 'POST':
        ...
        if not has_reservation:
            # Rediriger avec un message d'erreur si l'utilisateur n'a pas de
            # réservation
            return redirect(
                reverse(
                    'stations:reservations',
        ...
        # Corrigé: utilise 'note' au lieu de 'rating'
        note = request.POST.get('note')
        # Corrigé: utilise 'commentaire' au lieu de 'review_text'
        commentaire = request.POST.get('commentaire')

        if note and commentaire:
            # Conversion en entier pour s'assurer que c'est un nombre
            existing_review.note = int(note)
...
def station_statistics(request, ID_Station, username):
    ...
    # Calcul des KPI
    reservations = Reservation.objects.filter(
        ID_Station=station_obj, time_start__gte=start_date)
    ...
    first_reservation = Reservation.objects.filter(
        ID_Station=station_obj).order_by('time_start').first()
    if period == 'day':
        total_hours = 24
    ...
    chart_data.update(
        {
            ...
            'hourly_counts': [
                next(
                    (entry['count'] for entry in hourly_data if entry['hour'] == hour),
                    0) for hour in range(24)],
            'weekdays': [
                'Lundi',
                'Mardi',
                'Mercredi',
            ...
            'weekday_counts': [
                next(
                    (entry['count'] for entry in weekday_data if entry['weekday'] == day),
                    0) for day in range(
                    1,
                    8)],
        })
    ...
    recent_reservations = Reservation.objects.filter(
        ID_Station=station_obj).select_related('username').order_by('-time_start')[:10]
