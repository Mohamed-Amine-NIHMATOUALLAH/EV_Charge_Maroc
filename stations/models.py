...
class station(models.Model):
    ...
    connector_types = models.CharField(
        max_length=255,
        blank=True,
        null=True)  # Ex. "Type 2, CCS"
...
class Reservation(models.Model):
    ...
    def get_qr_code_data(self):
        ...
        return f"RESERVATION:{
            self.get_formatted_id()}|STATION:{
            self.ID_Station.ID_Station}|DATE:{
            self.time_start.strftime('%Y-%m-%d %H:%M')}|DUREE:{
                self.duration}"
    ...
    def __str__(self):
        return f"Reservation {
            self.ID_Reservation} at {
            self.ID_Station.nom} by {
            self.username}"
