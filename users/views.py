...
class ProprietaireVE_View_creat(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Créer un dictionnaire de contexte pour stocker les erreurs et les
        # valeurs soumises
        context = {
            'form': {
        ...
        if not terms:
            context['form']['terms']['errors'].append(
                "Vous devez accepter les conditions d'utilisation.")
            has_error = True

        if not all([CIN, first_name, last_name, username, email,
                   phone_number, password, password_confirmation]):
            context['form']['non_field_errors'].append(
                "Veuillez remplir tous les champs.")
            has_error = True

            # Vérifier chaque champ individuellement pour afficher des erreurs
            # spécifiques
            if not CIN:
                context['form']['CIN']['errors'].append(
                    "Ce champ est obligatoire.")
            if not first_name:
                context['form']['first_name']['errors'].append(
                    "Ce champ est obligatoire.")
            if not last_name:
                context['form']['last_name']['errors'].append(
                    "Ce champ est obligatoire.")
            if not username:
                context['form']['username']['errors'].append(
                    "Ce champ est obligatoire.")
            if not email:
                context['form']['email']['errors'].append(
                    "Ce champ est obligatoire.")
            if not phone_number:
                context['form']['phone_number']['errors'].append(
                    "Ce champ est obligatoire.")
            if not password:
                context['form']['password']['errors'].append(
                    "Ce champ est obligatoire.")
            if not password_confirmation:
                context['form']['password_confirmation']['errors'].append(
                    "Ce champ est obligatoire.")
        ...
        if ProprietaireVE.objects.filter(
                CIN=CIN).exists() or Fournisseur.objects.filter(
                CIN=CIN).exists():
        ...
        if ProprietaireVE.objects.filter(
                email=email).exists() or Fournisseur.objects.filter(
                email=email).exists():
            context['form']['email']['errors'].append(
        ...
        if ProprietaireVE.objects.filter(
                phone_number=phone_number).exists() or Fournisseur.objects.filter(
        ...
        if ProprietaireVE.objects.filter(
                username=username).exists() or Fournisseur.objects.filter(
...
class Fournisseur_View_creat(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if Fournisseur.objects.filter(
                CIN=CIN).exists() or ProprietaireVE.objects.filter(
                CIN=CIN).exists():
            context['form']['CIN']['errors'].append("Ce CIN est déjà utilisé.")
            has_error = True
        ...
        # Vérifier si l'email existe déjà
        if Fournisseur.objects.filter(
                email=email).exists() or ProprietaireVE.objects.filter(
                email=email).exists():
            context['form']['email']['errors'].append(
                "Cette adresse email est déjà utilisée.")
            has_error = True
        ...
        # Vérifier si le numéro de téléphone existe déjà
        if Fournisseur.objects.filter(
                phone_number=phone_number).exists() or ProprietaireVE.objects.filter(
                phone_number=phone_number).exists():
            context['form']['phone_number']['errors'].append(
                "Ce numéro de téléphone est déjà utilisé.")
            has_error = True
            ...
            if ProprietaireVE.objects.filter(email=email).exists():
                ...
                if user.check_password(password):
                    ...
                    return HttpResponseRedirect(
                        reverse('Home:Home', args=[user.username]))
                else:
                    context['form']['password']['errors'].append(
                ...
                context['form']['email']['errors'].append(
                    "Aucun compte propriétaire trouvé avec cet email.")
                has_error = True
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not has_error:
            if Fournisseur.objects.filter(email=email).exists():
                ...
                context['form']['email']['errors'].append(
                    "Aucun compte fournisseur trouvé avec cet email.")
                has_error = True
...
def profile(request, username):
    ...
    if request.method == 'POST':
        ...
        if action == 'update_info':
            ...
            # Utiliser la valeur actuelle si non fournie
            CIN = request.POST.get('CIN', user.CIN)
...
def password_reset_request(request):
    if request.method == "POST":
        email = request.POST.get('email')
        try:
            # Utiliser Users au lieu de User
...
def password_reset_verify(request, code):
    ...
    try:
        ...
        if reset_code_obj.is_expired():
            ...
            return render(
                request, 'password_reset_verify.html', {
                    'error': 'Ce code a expiré. Veuillez demander un nouveau code.'})
    except PasswordResetCode.DoesNotExist:
        return render(
            request, 'password_reset_verify.html', {
                'error': 'Code de vérification invalide. Veuillez vérifier votre email ou demander un nouveau code.'})
    ...
    if request.method == "POST":
        ...
        if step == 'verify_code':
            ...
            if verification_code == code:
                # Code vérifié, afficher le formulaire de réinitialisation du
                # mot de passe
        ...
        # Étape 2: Réinitialisation du mot de passe
            ...
            try:
                ...
                if len(new_password) < 8:
                    ...
                    return render(
                        request, 'password_reset_verify.html', context)
