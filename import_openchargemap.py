# Notez que nous utilisons "station" (minuscule) car c'est le nom de la classe
...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'EV_Charge_Maroc.settings')
...
def fetch_all_stations():
    ...
    while True:
        ...
        print(
            f"Nombre de stations récupérées jusqu'à présent : {
                len(all_stations)}")
...
for station_data in stations:
    ...
    connector_types = ", ".join([conn.get("ConnectionType", {}).get(
        "Title", "Inconnu") for conn in connections])
    puissance = max([conn.get('PowerKW', 0.0)
                    for conn in connections if conn.get("PowerKW")], default=0.0)
    ...
    if station.objects.filter(adresse=adresse).exists():
        print(
            f"Station ignorée (adresse déjà existante) : {nom}, Adresse: {adresse}")
        continue
    ...
    station.objects.create(
        ...
        operator=operator,
        puissance=puissance if puissance else "Non spécifié",
...
print(
    f"Importation terminée. Total de stations importées : {
